...
class Switchwindow:
    ...
    def __init__(self,master,name,size=(Rpipanel.width,Rpipanel.height)):
        global cnf,scrsize,scrmode
        self.master=master
        ...
        self.bpadx=cnf.dml["global"]["btn_pad"]["x"]
        self.bpady=cnf.dml["global"]["btn_pad"]["y"]
        self.monofont=cnf.dml['global']['font']
        self.fontsize=cnf.dml['global']['font']["fonts_size"][scrmode]
...
class Switchpanel:
    ...
    def __init__(self,master,name,size=(scrsize[0],scrsize[1]),app=Rpipanel):
        global cnf,scrsize,scrmode
        self.master=master
        self.window_name=name
        self.app=app
    ...
    def run_animation(self,period=1000):
        if self.run:
            if self.animation != None:
                self.animation()
            self.app.window.after(period,self.run_animation)
...
class Conf(Switchpanel):
    ...
    def __init__(self, master, name, buttons=None, app=Rpipanel):
        Switchpanel.__init__(self,master,name,app=app)
        if buttons == None:
            self.buttons = [ [ "BACK", partial(_clock,Rpipanel) ] ]
        else:
            self.buttons = buttons
        self.tb = Topbar(self,master,"tb",buttons)
        self.panels = {}
        for item in cnf.dml["conf"]["pannels"]:
            self.panels=app.panels["item"](self,master,item)
...
class P1(Switchpanel):
    """ test p1 """
    def __init__(self,master,name,app=Rpipanel):
        Switchpanel.__init__(self,master,name,app=app)
        frm = tk.Frame(self.fram,relief=tk.FLAT, bg="green",width=100, height=100, )
        frm.place(relx=0.5, rely=0.5, anchor=tk.CENTER)
        lbl=tk.Label(frm,text="P1",justify=tk.CENTER,anchor=tk.CENTER, font=(self.monofont,self.fontsize) )

class P2(Switchpanel):
    """ test p3 """
    def __init__(self,master,name,app=Rpipanel):
        Switchpanel.__init__(self,master,name,app=app)
        frm = tk.Frame(self.fram,relief=tk.FLAT, bg="orange",width=100, height=100, )
        frm.place(relx=0.5, rely=0.5, anchor=tk.CENTER)
        lbl=tk.Label(frm,text="P2",justify=tk.CENTER,anchor=tk.CENTER, font=(self.monofont,self.fontsize) )

class P3(Switchpanel):
    """ test p3 """
    def __init__(self,master,name,app=Rpipanel):
        Switchpanel.__init__(self,master,name,app=app)
        frm = tk.Frame(self.fram,relief=tk.FLAT, bg="lightblue",width=100, height=100, )
        frm.place(relx=0.5, rely=0.5, anchor=tk.CENTER)
        lbl=tk.Label(frm,text="P3",justify=tk.CENTER,anchor=tk.CENTER, font=(self.monofont,self.fontsize) )
...
class Rpipanel:
    ...
    def run(self):
        ...
        #Rpipanel.frames["conf"] = tk.Frame(self.window,relief=tk.FLAT,borderwidth=0,bg=self.cnf.dml["global"]["frm_bg"],width=self.scrsize[0],height=self.scrsize[1])
...
def _clock(app=Rpipanel, event=None):
    """ switch the clock frame to top """
    app.frames["panel"].pack_forget()
    app.frames["tb"].pack_forget()
    app.frames["clock"].pack(fill=tk.BOTH)


def _unlock(app=Rpipanel, event=None):
    """ switch the lock frame off, swich the main frame to top and relise animations """
    app.frames["lock"].pack_forget()
    app.frames["clock"].pack(fill=tk.BOTH)
    _clock()

def _lock(app=Rpipanel, event=None):
    """ switch the lock frame on, hold animations and swich off curent frame """
    app.frames["tb"].pack_forget()
    app.frames["panel"].pack_forget()
    app.frames["clock"].pack_forget()
    app.state["lock"].clear()
    app.frames["lock"].pack(fill=tk.BOTH)
